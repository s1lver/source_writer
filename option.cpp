...
AnsiString lng2;
//переменна€ им€ макроса
AnsiString FileName;
//переменна€ пользовательского €зыка
AnsiString usrlng;
...
//загружаем в макрос лист значени€ из файлов
...
Options->ќтладчик->Caption=Inilng->ReadString("form","opt21","");
...
{//активируем поле ввода времени сохранени€
...
ShowMessage("Ќе найдена библиотека Lua");
} else
...
ShowMessage("Ќе найдена библиотека XML");
} else
...
ShowMessage("Ќе найдена библиотека Error");
} else
...
{
//добавл€ем пользовательский €зык
...
{
//устанавливаем пользовательский €зык интерфейса
